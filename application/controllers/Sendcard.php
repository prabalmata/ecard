...
class Sendcard extends CI_Controller {
    ...
    public function index() {
		$id = $_GET['card_id'];
		$card= $this->user->get_card_detail_by_card_id($id);
         $single_card['details'] = $card[0];
         $single_card['company_list'] = $this->user->get_all_company(); 
        $this->load->view('header');
        $this->load->view("card_universal",$single_card);
    }
    public function ajax_send_mail(){
		...
		$insert_id = $this->UserEmail->user_email_dispatch_save_non_login($dataArr);
		$insertArr = $this->UserEmail->get_non_login_send_data($insert_id);
		...
		$admincard = $this->card->get_card_non_login($insertRow->from_admin_card);
		...
		$url  = base_url().'index.php/sendcard/preview_path?card_id='.$insertRow->from_admin_card.'&company_name='.$insertRow->from_company;
		...
		if($insertRow->lang == 'en'){
			...
			$message = str_ireplace('[company]',$insertRow->from_title, $message);
